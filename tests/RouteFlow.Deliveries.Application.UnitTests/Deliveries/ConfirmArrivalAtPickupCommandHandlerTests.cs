...
public sealed class ConfirmArrivalAtPickupCommandHandlerTests
{
    [Fact]
    public async Task Handle_WhenDeliveryIsDispatched_ShouldConfirmArrivalAndPersistChanges()
    {
        // Arrange
        ...
        var handler = new ConfirmArrivalAtPickupCommandHandler(
            repository,
            new FixedTimeProvider(DeliveryTestData.Now));

        var command = new ConfirmArrivalAtPickupCommand(delivery.Id);
    ...
    public async Task Handle_WhenDeliveryDoesNotExist_ShouldThrowAndNotPersist()
    {
        ...
        var handler = new ConfirmArrivalAtPickupCommandHandler(
            repository,
            new FixedTimeProvider(DeliveryTestData.Now));
        var command = new ConfirmArrivalAtPickupCommand(deliveryId);
