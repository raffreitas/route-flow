...
public sealed class UpdateAddressBeforePickupCommandHandlerTests
{
    [Fact]
    public async Task Handle_WhenPackageIsAtMerchant_ShouldUpdateAddressAndPersistChanges()
    {
        // Arrange
        ...
        var handler = new UpdateAddressBeforePickupCommandHandler(
            repository,
            new FixedTimeProvider(DeliveryTestData.Now));

        // Act
        await handler.HandleAsync(
            new UpdateAddressBeforePickupCommand(delivery.Id, newAddress),
    ...
    public async Task Handle_WhenDeliveryDoesNotExist_ShouldThrowAndNotPersist()
    {
        ...
        var exception = await Assert.ThrowsAsync<DeliveryNotFoundException>(() =>
            handler.HandleAsync(
                new UpdateAddressBeforePickupCommand(deliveryId, CreateNewAddress()),
                CancellationToken.None));
    ...
    public async Task Handle_WhenDomainRejectsTransition_ShouldThrowAndNotPersist()
    {
        ...
        await Assert.ThrowsAsync<DomainException>(() => handler.HandleAsync(
            new UpdateAddressBeforePickupCommand(delivery.Id, CreateNewAddress()),
            CancellationToken.None));
        await repository.DidNotReceive().SaveChangesAsync(Arg.Any<CancellationToken>());
    }
