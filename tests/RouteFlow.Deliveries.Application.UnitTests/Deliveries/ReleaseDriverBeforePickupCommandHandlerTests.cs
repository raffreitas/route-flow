...
public sealed class ReleaseDriverBeforePickupCommandHandlerTests
{
    [Fact]
    public async Task Handle_WhenDriverIsAssigned_ShouldReleaseDriverAndPersistChanges()
    {
        // Arrange
        ...
        var handler = new ReleaseDriverBeforePickupCommandHandler(
            repository,
            new FixedTimeProvider(DeliveryTestData.Now));

        // Act
        await handler.HandleAsync(
            new ReleaseDriverBeforePickupCommand(delivery.Id, "Driver unavailable"),
    ...
    public async Task Handle_WhenDeliveryDoesNotExist_ShouldThrowAndNotPersist()
    {
        ...
        var exception = await Assert.ThrowsAsync<DeliveryNotFoundException>(() =>
            handler.HandleAsync(
                new ReleaseDriverBeforePickupCommand(deliveryId, "Driver unavailable"),
                CancellationToken.None));
